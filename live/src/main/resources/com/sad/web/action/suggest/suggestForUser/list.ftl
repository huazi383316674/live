...
[@b.grid items=suggests var="suggest"]
	...
	[@b.row]
		[@b.boxcol/]
		[@b.col title="编号" property="no" width="7%"]
			[@b.a href='!info?suggest.id=${suggest.id}' title='查看意见详细信息']${(suggest.no)!}[/@]
		[/@]
		[@b.col title="提出人" property="speaker.fullname" width="9%"][/@]
		[@b.col title="所属科室" property="speaker.department.name" width="9%"][/@]
		...
		[#--[@b.col title="指派人" property="type.name" /]--]
		[@b.col title="处理情况" property="handleType.name" width="8%"]
			[#if suggest.handleType.id = 1]<font color="red">${(suggest.handleType.name)!}</font>
		...
		<table width="100%">
			...
			<tr>
				<td colspan="2" align="center" height="40px">
					...
					<input type="hidden" name="zgexamSeason.id" value="${(zgexamSeason.id)!}" />
					<input type="hidden" name="signUpIds" value="" />
				</td>
			</tr>
		</table>
		[/@]
