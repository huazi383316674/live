...
[@b.form name="recordEditForm_" action="!save" title="" theme="list" target="contentDiv"]
...
<script>
	jQuery(function(){
		jQuery.colorbox({transition:'none', title:"同步收支清单", overlayClose:false, width:"650px", height:"450px", inline:true, href:"#inPayByReturnDiv"});
	...
	function saveAll() {
		...
		jQuery(":input[name=record\\.name]", document.recordEditForm_).require();
		jQuery(":input[name=record\\.amount]", document.recordEditForm_).require();
		jQuery(":input[name=businessType\\.id]", document.recordEditForm_).require();
		jQuery(":input[name=record\\.type\\.id]", document.recordEditForm_).require();
		jQuery(":input[name=record\\.createOn]", document.recordEditForm_).require();
		res = jQuery.validity.end().valid;
		if(false == res) return;
		
		bg.form.transferParams(document.debtReturnEditForm, document.recordEditForm_, null, false);
		bg.form.submit(document.recordEditForm_, "debtReturn!save.action");
