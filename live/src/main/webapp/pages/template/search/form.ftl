[#ftl]
<script>function buildTime(form) { bg.form.addInput(form, "_", new Date().getTime()); } </script>

<form id="${tag.id}" name="${tag.name}" action="${tag.action}" method="post"[#if tag.target??] target="${tag.target}"[/#if][#if tag.onsubmit??] onsubmit="${tag.onsubmit};buildTime(this)"[#else] onsubmit="buildTime(this)"[/#if]>
